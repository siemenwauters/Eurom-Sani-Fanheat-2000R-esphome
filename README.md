...
When i found a little heater that comes from a decent brand on a local second hand site i jumped for the opportunity to get it at 15 euro since i did not need the warranty anyway with my modifications.
...
The good news is that the interface between these 2 boards is so simple that we can simply replace the complete control module with the esp32 and a relay board ( you could use something else, relays are just simple to keep things separated ). make sure to keep the screws since i used those to mount my relay and esp board.
